...
# Screenshots will be saved to ~/Pictures/Screenshots by default
...
# Screenshot filename (uses timestamp)
timestamp=$(date +"%Y-%m-%d_%H-%M-%S")
NAME="screenshot_${timestamp}.png"

# Screenshot editor (change to your preferred editor)
...
# Rofi CMD
rofi_cmd() {
  rofi -dmenu -i -no-show-icons -p "$prompt"
}
...
# CMD
timer_cmd() {
  rofi -dmenu -i -no-show-icons -p "Choose timer"
}
...
# CMD
type_screenshot_cmd() {
  rofi -dmenu -i -no-show-icons -p "Type of screenshot"
}
...
copy_save_editor_cmd() {
  rofi -dmenu -i -no-show-icons -p "How to save"
}
...
takescreenshot() {
  sleep 1
  grimblast --notify "$option_chosen" "$option_type_screenshot" "$screenshot_dir/$NAME"
}
